...
The pipeline extracts 8 CSV files from Kaggle and interest rate/exchange rate data from Brazilian Central Bank , loads them into Google BigQuery using Meltano, and is designed for future transformation with dbt.

**Workflow**:

- ✅ Extract (Kaggle API) and (Brazilian Central Bank REST API)
...
1.  **Extract**: Downloads raw CSV files from Kaggle and load into Postgre Database.
2.  **Load**: Ingests data from Postgre Database and  Brazilian Central Bank REST API into Google BigQuery using Meltano.
3.  **Transform**: Uses dbt to model data into staging, fact, and dimension tables.
